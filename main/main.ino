...
/** 
 * 未发布  
 * 1. 关闭眨眼这个效果，不然某些表情看起来有点怪  100%
 * 2. 音量默认改为 100%
 * 3. 电量检测不准    ing...
 * 3. 接入摄像头功能    ing...
 * 4. 接入TFT彩屏      ing...
 * 5. 显示对话内容优化（分批显示内容）  ing...
 * 6. AI 作图   ing...
 * 7. 声纹识别   ing...
 * 8. 蓝牙配网   ing...
 **/
...
#define IS_ESP_AI_S3  // ESP-AI S3 开发板（OLED 屏）
...
const eEmotions moods[] = {
  ...
  eEmotions::Angry,      // 愤怒的
  eEmotions::Sad,        // 伤心
  eEmotions::Surprised,  // 意外
  // eEmotions::Glee,         // 快乐
  eEmotions::Happy,  // 快乐的
  // eEmotions::Worried,      // 发愁的
  // eEmotions::Focused,      // 专注的
  // eEmotions::Annoyed,      // 恼怒的
  // eEmotions::Skeptic,      // 持怀疑态度
  // eEmotions::Frustrated,   // 懊恼的
  // eEmotions::Unimpressed,  // 不感兴趣
  // eEmotions::Sleepy,       // 困倦的
  // eEmotions::Suspicious,   // 感觉可疑的，怀疑的；不信任的
  eEmotions::Squint,  // 眯眼看
  // eEmotions::Furious,      // 狂怒的 热烈兴奋的
  // eEmotions::Scared,       // 惊恐的，恐惧的；担心的，焦虑的
  // eEmotions::Awe,          // 敬畏
};
...
long emotion_start_time = 0;
// 情绪灯光灯珠数量
int emotion_led_number = 150;
// 情绪灯光
Adafruit_NeoPixel pixelsEmotion(emotion_led_number, 45, NEO_GRB + NEO_KHZ800);
...
void onError(String code, String at_pos, String message) {
  ...
  //   while (true) {
  //     // 淡曙红
  //     pixels.setPixelColor(0, pixels.Color(238, 39, 70));
  //     pixels.setBrightness(100);  // 亮度设置
  //     pixels.show();
  //     delay(500);
  //     pixels.clear();
  //     pixels.show();
...
void on_command(String command_id, String data) {
  ...
  if (command_id == "add_volume") {
    ...
    String realStr = "";
    if (data.length() > 100) {
      realStr += data.substring(0, 100);
      realStr += "...";
    } else {
      realStr = data;
    }
    face->DrawStrAni(realStr, false, 0);
#endif
  } else if (command_id == "ota_update") {
    Serial.println("触发升级啦！");
    // test...
    // update_ota("https://api.espai2.fun/public/ota/test/1.bin");
...
void faceTask(void *arg) {
  while (true) {
    // OTA 升级时更新慢一点
    if (start_update_ed == true) {
#if !defined(IS_ESP_AI_S3_NO_SCREEN)
      face->DrawStr("系统升级中:" + ota_progress, -50);
// face->Update();
#endif
      vTaskDelay(500 / portTICK_PERIOD_MS);
    } else if (is_yw_connected) {
#if !defined(IS_ESP_AI_S3_NO_SCREEN)
      // 其他情况正常更差更新
      face->Update();
#endif
...
void emotionTask(void *arg) {
  while (true) {
    ...
    if (prve_emotion != emotion) { 
      if (emotion == "快乐") {
        pixelsEmotion.clear();
        for (int i = 0; i < emotion_led_number; i++) {                         // For each pixel...
          pixelsEmotion.setPixelColor(i, pixelsEmotion.Color(255, 255, 255));  // 白色
          pixelsEmotion.show();
          delay(5);
        }
      } else if (emotion == "伤心") {
        pixelsEmotion.clear();
        for (int i = 0; i < emotion_led_number; i++) {                       // For each pixel...
          pixelsEmotion.setPixelColor(i, pixelsEmotion.Color(255, 255, 0));  // 黄色
          pixelsEmotion.show();
          delay(5);
        }
      } else {
        ...
        // 全部作为无情绪处理：默认米黄色
        pixelsEmotion.clear();
        for (int i = 0; i < emotion_led_number; i++) {  
          pixelsEmotion.setPixelColor(i, pixelsEmotion.Color(250,138,0));
          pixelsEmotion.show();
          delay(5);
        }
      }
...
void setup() {
  ...
  face = new Face(/* screenWidth = */ 128, /* screenHeight = */ 32, /* eyeSize = */ 40, 42, 41, /* 屏幕类型 = */ "091", /* 屏幕旋转角度 = */ 0);
  // 音量减
  pinMode(vol_add, INPUT_PULLUP);
  // 音量加
  pinMode(vol_subtract, INPUT_PULLUP);
#endif

#if defined(IS_XIAO_ZHI_S3_3)
  face = new Face(/* screenWidth = */ 128, /* screenHeight = */ 64, /* eyeSize = */ 40, 42, 41, /* 屏幕类型 = */ "096", /* 屏幕旋转角度 = */ 0);
  // 音量减
  pinMode(vol_add, INPUT_PULLUP);
  // 音量加
  pinMode(vol_subtract, INPUT_PULLUP);
...
#if defined(IS_ESP_AI_S3)
  face = new Face(/* screenWidth = */ 128, /* screenHeight = */ 64, /* eyeSize = */ 40, oled_sck.toInt(), oled_sda.toInt(), /* 屏幕类型 = */ "096", /* 屏幕旋转角度 = */ 0);
#endif
  ...
  // 更新文字
  // face->DrawStr("正在初始化...", -30);
  face->DrawStr("ESP-AI V" + _version, -45);

  // 低于 8Kb 的话很容易卡死
  xTaskCreate(faceTask, "faceTask", 1024 * 8, NULL, 1, NULL);
#endif
...
// 小智AI 引脚不一样
  ...
  // 情绪灯光任务
  xTaskCreate(emotionTask, "emotionTask", 1024 * 2, NULL, 1, NULL);
...
#if defined(IS_XIAO_ZHI_S3_2) || defined(IS_XIAO_ZHI_S3_3)
int esp_ai_debounce_delay = 60;
// 音量控制
long esp_ai_last_debounce_time_add = 0;
int esp_ai_prev_state_add = 0;
long esp_ai_last_debounce_time_sub = 0;
int esp_ai_prev_state_sub = 0;
#endif
...
void loop() {
  ...
  if (start_update_ed == false) {
...
#if defined(IS_XIAO_ZHI_S3_2) || defined(IS_XIAO_ZHI_S3_3)
    // 加音量
    int reading_add = digitalRead(vol_add);
    long curTime = millis();
    if (reading_add == 0) {
      if ((curTime - esp_ai_last_debounce_time_add) > esp_ai_debounce_delay) {
        esp_ai_last_debounce_time_add = curTime;
        if (esp_ai_prev_state_add != reading_add) {
          esp_ai_prev_state_add = reading_add;
          up_click(0.1);
        }
      }
    } else {
    ...
    // 减音量
    int reading_sub = digitalRead(vol_subtract);
    if (reading_sub == 0) {
      if ((curTime - esp_ai_last_debounce_time_sub) > esp_ai_debounce_delay) {
        esp_ai_last_debounce_time_sub = curTime;
        if (esp_ai_prev_state_sub != reading_sub) {
          esp_ai_prev_state_sub = reading_sub;
          down_click(0.1);
        }
      }
    } else {
