 ...
 * 自动检测 OTA 升级函数
 */
void auto_update(String device_id, String api_key, String bin_id, String is_official, String domain, String version, Face &face, ESP_AI &esp_ai, ESPOTAManager &otaManager)
{
    face.SetChatMessage("系统检查升级中。");
    wait_mp3_player_done();
    mp3_player_write(jian_ce_shen_ji_mp3, jian_ce_shen_ji_mp3_len);
    ...
    if (httpCode > 0)
    {
        ...
        if (JSON.typeof(parse_res) == "undefined" || String(httpCode) != "200")
        {
            ...
            wait_mp3_player_done();
            mp3_player_write(shen_ji_shi_bai_mp3, shen_ji_shi_bai_mp3_len);
            wait_mp3_player_done();
        }
        else
        {
            ...
            if (parse_res.hasOwnProperty("success"))
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        wait_mp3_player_done();
                        mp3_player_write(zheng_zai_sheng_ji_mp3, zheng_zai_sheng_ji_mp3_len);
                        wait_mp3_player_done();
